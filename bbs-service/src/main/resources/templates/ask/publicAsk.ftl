<!DOCTYPE html>
<html lang="en">
<head>
<#include "../include/top.ftl">
<link rel="stylesheet" type="text/css" href="${realpath}/css/publicAsk.css">
	<script type="text/javascript" src="${realpath}/ueditor/ueditor.config.js"></script>
	<script type="text/javascript" src="${realpath}/ueditor/ueditor.all.js"></script>
	<script type="text/javascript" charset="utf-8" src="${realpath}/ueditor/lang/zh-cn/zh-cn.js"></script>
	<script type="text/javascript" charset="utf-8" src="${realpath}/ueditor/MyActionUrl.js"></script>
	<meta charset="UTF-8">
	<title>提问</title>
</head>
<body>
<#include "../include/header.ftl">
	<div class="main">
		<div class="container">
		<form id="postBbsForm">
		<input type="hidden" name="content" id="richContent" />
			<div class="row">
				<div class="form-group" id="topicTitle">
					<label class="col-xs-1 control-label"> <em class="important">*</em>
						标题:
					</label>
					<div class="col-xs-10">
						<input class="form-control"  name="title" id="title"></input>
					</div>
				</div>
			</div>

	
			<div class="row">
				<div class="form-group" id="ueContent">
					<label class="col-xs-1 control-label"> <em class="important">*</em>
						内容:
					</label>
					<div class="col-xs-10">
						<script id="editor" type="text/plain" style="width:950px;height:300px;"></script>
					</div>
				</div>
			</div>
			
			
			<div class="row">
				<div class="form-group" id="needMark">
					<label class="col-xs-1 control-label">&nbsp;&nbsp;&nbsp;
						积分:
					</label>
					<div class="col-xs-1">
						<input class="form-control"  name="mark" id="mark" value="0"></input>
					</div>
					<div class="col-xs-4">
					 	<span class="info">你还有：<span class="left-mark" id="my-mark">${myCount}</span>&nbsp;分，打赏积分回答的人会更多哦。</span>
					 </div>
				</div>
			</div>
		</form>


		<div class="row">
			<div class="col-xs-10"></div>
			<button class="btn btn-info" id="postExamBtn">发表问题</button>
		</div>

		</div>

	</div>
<script type="text/javascript" src="${realpath}/js/publicAsk.js"></script>

<#include "../include/footer.ftl">	
</body>
</html>