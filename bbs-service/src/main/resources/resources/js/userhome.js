...
function dispatchLoad(type) {
	...
	function loadShuoShuos(pageNum){
		...
		$("#load-more").remove();
    	$('<div id="loading"> <div class="loading-con"><img src="' + fzqblog.realpath + '/images/loading.gif"/><span>正在加载.......</span></div></div>').appendTo(tagcontent);
		$.ajax({
...
function topicItem(data){
    ...
    $('<a class="topic-cate-tag"  target="_blank"  href="'+zuul.realpath+'/bbs/sub_board/'+data.categoryId+'"><span>'+data.categoryName+'</span></a>').appendTo(topic_item_title);
...
function loadFans(){
    ...
    $.ajax({
	 ...
	 dataType: 'json',
	    ...
	    for (var i = 0, _len = list.length, d; i < _len, d = list[i]; i++) {
		$("<li><a href='"+fzqblog.realpath+"/user/"+d.userId+"'><img  src='"+fzqblog.realpath+"/images/"+d.userIcon+"'><span>"+d.userName+"</span></a></li>").appendTo(topic_content);
	    }
	}
    });
}
...
function loadFocus(){
    ...
    $.ajax({
		...
		success:function(res){
		    ...
		    for (var i = 0, _len = list.length, d; i < _len, d = list[i]; i++) {
			$("<li><a href='"+fzqblog.realpath+"/user/"+d.friendUserId+"'><img   src='"+fzqblog.realpath+"/images/"+d.friendUserIcon+"'><span>"+d.friendUserName+"</span></a></li>").appendTo(topic_content);
		    }
		}
    });
}