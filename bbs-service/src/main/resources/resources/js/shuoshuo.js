...
var uploader = WebUploader.create({
	auto: true,
	swf: fzqblog.realpath+ '/webuploader/Uploader.swf',
...
function ImageItem(img) {
	...
	$('<a href="javascript:void(0)"><img src="' + fzqblog.realpath + '/images/icon/icon-del.png"></a>').appendTo(span);
	return span;
}
...
$(document).on("click", ".op-at", function() {
	...
	function showEmotion(targetObj, textarea){
		...
		for (var i = 0, _len = emotions.length; i < _len; i++) {
			var item = $("<div data=" + emotions[i] + " class='emotion' title=" + emotions[i] + "><img src='" + fzqblog.realpath +"/images/emotions/" + i + ".gif'></div>").appendTo(emotion_panel).bind("click", function() {
				d.close();
			...
			success:function(res){
				...
				if (_len == 0) {
					...
					for (var i = 0, item; i < _len, item = list[i]; i++) {
						$("<a href='javascript:;' class='at_user'><img src='" + fzqblog.realpath + "/images/" + item.friendUserIcon + "' class='img-thumbnail' style='width:40px;height:40px;'><span>" + item.friendUserName + "</span></a>").appendTo(content).click(function() {
							d.close();
	...
	function publicShuoShuo(){
		...
		var d = dialog({
							content: "<div><img src='" + fzqblog.realpath +"/images/loading.gif' />&nbsp;&nbsp;&nbsp;发布中......</div>",
						});
						d.showModal();
						setTimeout(function() {
							d.close().remove();
	...
	function ShuoShuoItem(data){
		...
		$('<a href="../user/' + data.userId+ '"><img src="' + fzqblog.realpath + '/images/'+ data.userIcon + '" class="cd-timeline-img img-circle"></a>').appendTo(item);
	...
	function LikeItem(like){
		var likeItem = $('<a href="' + fzqblog.realpath + '/user/' + like.userId + '" class="like-info"><img class="user-like-info img-responsive" src="' + fzqblog.realpath + '/images/' + like.userIcon + '"></a>');
		return likeItem;
	}
	function CommentItem(comment){
		...
		$('<a href="' + fzqblog.realpath + '/user/' + comment.userId + '"><img src="' + fzqblog.realpath + '/images/' + comment.userIcon + '" class="user-icon-image img-circle"></a>').appendTo(commentusericon);
