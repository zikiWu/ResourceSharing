...
//						var d = dialog({
//							content: "<div><img src='" + fzqblog.realpath +"/images/loading.gif' />&nbsp;&nbsp;&nbsp;注册成功,登录中......</div>",
//						});
//						d.showModal();
//						setTimeout(function() {
//							d.close().remove();
	...
	*/
	...
	$('#login').click(function(event) {
		...
		if (account == null || $.trim(account) == '') {
			...
			$.ajax({
				url: zuul.realpath + 'user/login.do',
				type: 'POST',
				dataType: 'json',
					...
					if (data.errorMsg == null) {
						...
						setTimeout(function() {
							...
							if (null == fzqblog.redirect || fzqblog.redirect == "") {
								fzqblog.redirect = zuul.realpath;
							    }
...
//											if (data.errorMsg == null) {
//												var d = dialog({
//													content: "<div><img src='" + fzqblog.realpath +"/images/loading.gif' />&nbsp;&nbsp;&nbsp;修改成功,跳转中...</div>",
//												});
//												d.showModal();
//												setTimeout(function() {
//													d.close().remove();
...
//控制用户信息显示  加载用户签到信息
	$("#userheadicon").mouseenter(function(){
		$.ajax({
	 		url:  zuul.realpath + '/common/loadSignInfo',
	 		type: 'POST',
	 		dataType: 'json',
	 ...
	 $(".unsigninimage").click(function(event) {
			$.ajax({
				...
				success:function(result){
					...
					content: "<div><img src='" + fzqblog.realpath +"/images/signin.gif' /></div>",
				});
					d.showModal();
					setTimeout(function() {
						d.close().remove();
...
function showEmotion(targetObj, textarea){
	...
	for (var i = 0, _len = emotions.length; i < _len; i++) {
		var item = $("<div data=" + emotions[i] + " class='emotion' title=" + emotions[i] + "><img src='../images/emotions/" + i + ".gif'></div>").appendTo(emotion_panel).bind("click", function() {
			d.close();
...
function goLogin() {
	var url = fzqblog.curUrl;
	url = encodeURI(url);
	document.location.href = zuul.realpath + "/common/login?redirect=" +zuul.realpath+ url;
}