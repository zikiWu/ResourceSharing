...
fzqblog.url = {
	...
	delMessage: zuul.realpath + "/common/delMessage",
	markMessage: zuul.realpath + "/common/markMessageRead"
}
$(function() {
	demo(1);
...
function del(messageId) {
	...
	var d = dialog({
		content: "<div><img src='" + fzqblog.realpath + "/images/loading.gif' />&nbsp;&nbsp;&nbsp;删除中...</div>",
	});
	d.showModal();
	setTimeout(function() {
		d.close().remove();
...
function mark(messageId) {
	...
	var d = dialog({
		content: "<div><img src='" + fzqblog.realpath + "/images/loading.gif' />&nbsp;&nbsp;&nbsp;标记中...</div>",
	});
	d.showModal();
	setTimeout(function() {
		d.close().remove();
	...
	$.getJSON(fzqblog.url.loadMessage, {
		...
		if (list.length > 0) {
			for (var i = 0, _len = list.length, d; i < _len, d = list[i]; i++) {
				var edit = "";
				if (d.url != "") {
					edit = edit + '<a href="' + d.url + '" title="查看" target="_blank" class="read-btn" url="' + d.url + '" messageId="' + d.id + '"><i class="icon i-preview"></i>&nbsp;&nbsp;&nbsp;&nbsp;'
				}
