<nav class="navbar navbar-inverse">
	<div class="container-fluid">
		<ul class="nav navbar-nav">
			...
			<form class="navbar-form navbar-left" role="search" id="searchForm"  method="POST">
				<select class="form-control" id="select" name="articleType">
					<option value="F" <#if articleType??&&articleType=='T'>selected="selected"</#if>>讨论区</option>
				</select>
				<div class="form-group">
					<input type="text" class="form-control" name="keyword" id="keyWord" value="${keyword!''}" style="width: 300px;">
				</div>
				<a class="btn btn-info" id="searchBtn2" href="javascript:void(0)">搜索</a>
			</form>
		</ul>
		...
		<ul class="nav navbar-nav navbar-right">
			<#if !user?? >
				...
				<li><a href="${zuulpath}/common/register">加入我们</a></li>
				<#assign userIcon = "">
				<#assign userName = "">
				<#assign userid = "">
			<#else>
							...
							<div class="panel-body">
								...
								<div class="user-action">
									...
									<div class="usre-action-panel">
										<a href="${realpath}/common/logout">退出</a>
									</div>
								</div>
							</div>
						</div>
